...
```{r plotPresidentsHeights, include = TRUE, eval = TRUE}
...
pics <- dir(paste0(getwd(),"/pics"), "*.png$")

pics <- paste0("./pics/",pics)
...
for (i in 1:length(pics)) {
  ...
  p <- p +  annotation_custom(grob = x,
                              ymin= PressHeight[i]+.01,
                              ymax= PressHeight[i]+1,
                              xmin = as.numeric(listofPresidents$MeanTermDate[i]- 200),
                              xmax = as.numeric(listofPresidents$MeanTermDate[i]) + 200)
  p
  # assign(picNames[i],x)
  rm(x)
...
```{r plotCourt, include = TRUE, eval = TRUE}
